...
export LANG=en
bin=/data/bin/python2/bin/$binr
pfx=$st/com.googlecode.$py4a
ext=$pfx/extras/$binr
PYTHONUSERBASE=$pfx/local
PYTHONPATH=$ext
PYTHONPATH=${PYTHONPATH}:$PYTHONUSERBASE
PYTHONPATH=${PYTHONPATH}:/data/bin/python2/lib/python$ver/lib-dynload
export PYTHONPATH
export PYTHONUSERBASE
export TEMP=$ext/tmp
export PYTHON_EGG_CACHE=$TEMP
export PYTHONHOME=/data/bin/python2
export LD_LIBRARY_PATH=/data/bin/python2/lib
cd $PW
if [ $api -lt 14 ]; then
